...
A standalone demonstration module that implements a **3-model adaptive ensemble** (XGBoost + HistGradientBoosting + ExtraTrees) for fraud detection within a multi-round adversarial simulation. Part of the [Enhanced Multi-Round Promo Fraud Detection](../PROJECT_SUMMARY.md) project.
...
2. Three ML models (XGBoost, HistGradientBoosting, ExtraTrees) are trained each round on 17 handcrafted features.
...
└── adaptive-ensemble-detector/ ← THIS MODULE (3-model ensemble + ADS on synthetic data)
```
...
| **Models** | XGB-SP (XGBoost on GNN embeddings) | XGBoost + HistGradientBoosting + ExtraTrees (tabular) |
| **Ensemble** | Single model (XGBoost) | 3-model weighted ensemble + ADS |
...
A single decision tree with one split on `email_disposable > 0.4` already separates most fraud from genuine. With 3 models and 17 features, the ensemble trivially achieves near-perfect classification.
...
- Generate GNN embeddings as feature vectors for the 3 models
...
└── backend/
    ...
    ├── adaptive_detector.py        # 3-model ensemble + ADS scoring
