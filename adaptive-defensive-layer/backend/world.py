...
class Account:
    __slots__ = (
        ...
        'tags', 'device_id', 'ip_id', 'detected_round', 'blocked',
        'blocked_round', 'decision', 'reviewed', 'risk',
    )
    ...
    def __init__(self, idx, attrs, label, creation_round, strategy='',
        ...
        # Adaptive Defensive Layer state (soft removal)
        self.blocked = False
...
class World:
    ...
    def block(self, idx, round_idx):
        """Soft-remove an account from the world (ADL ``Block`` decision).

        The account stays in ``accounts`` (indexes never shift) but is flagged
        ``blocked`` and therefore excluded from every future feature, metric,
        training split and generator seed. Blocked fraud can never reach the
        attacker again -- the feedback loop only sees survivors.
        """
        a = self.accounts[int(idx)]
        a.blocked = True
        a.blocked_round = int(round_idx)
        a.decision = 'block'
        return self

    def active_mask(self):
        """Boolean array over ``accounts``: True where the account is live."""
        return np.array([not a.blocked for a in self.accounts], dtype=bool)
