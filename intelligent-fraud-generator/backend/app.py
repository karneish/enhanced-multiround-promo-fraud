"""Flask HTTP API for the intelligent fraud generator add-on.

Endpoints
---------
GET  /api/health          - liveness + version
GET  /api/schema          - configuration schema + feature schema for the UI
...
GET  /api/report/<id>     - full report (metrics, generator stats, events)
GET  /api/history         - metadata of every run started this session
GET  /api/graph/<id>      - node/edge dump of the final graph for the explorer
"""
...
from .world import FEATURE_NAMES, INTRINSIC_NAMES, GRAPH_NAMES, INITIAL_STRATEGIES
...
@app.get('/api/health')
def health():
    return jsonify({'ok': True, 'service': 'intelligent-fraud-generator',
                    'time': time.time()})
...
def datasets():
    return jsonify([
        {'key': 'quick', 'label': 'Quick demo (3 rounds)',
         'config': {'rounds': 3, 'base_accounts': 300, 'initial_fraud': 40,
                    ...
                    'describe': 'Quick demo'}},
        {'key': 'default', 'label': 'Standard (8 rounds)',
         'config': {'rounds': 8, 'base_accounts': 500, 'initial_fraud': 60,
                    ...
                    'budget_pos': 6, 'budget_neg': 15,
                    'describe': 'Standard experiment'}},
        {'key': 'deep', 'label': 'Long evolution (10 rounds)',
